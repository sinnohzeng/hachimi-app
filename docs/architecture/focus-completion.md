...
| 2026-03-17 | P2 test coverage: (1) Extracted `notification_scheduling.dart` — pure functions for time calculation and ID generation, testable via `now` parameter injection. (2) Added `focus_timer_notifier_tick_test.dart` — 15 tests covering `_onTick`, `onAppResumed`, `_handleCountdownComplete`, `restoreSession`, `_shouldAutoComplete` via Clock injection. (3) Added `notification_scheduling_test.dart` — 15 tests covering `nextInstanceOfTime`, `nextInstanceOfWeekdayTime`, ID computation. |
