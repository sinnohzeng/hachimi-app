...
> - 2026-03-15 — 新增 §9.4 XP 系统共存说明：明确现有 XP 与冒险 XP 的独立性和不合并理由
...
v2.0 存在两套独立的 XP 系统，各自服务不同目的：

| 系统 | 存储位置 | 用途 | 增长来源 |
|------|---------|------|---------|
| **现有 XP**（`xp_service.dart`） | `materialized_state['xp']` | 成就解锁、猫咪进化阶段计算 | 专注分钟数直接累加 |
| **冒险 XP**（`adventure_xp`） | `materialized_state['adventure_xp']` | 冒险用户等级（Lv 1-20）、区域解锁、职业/专长门槛 | 专注分钟数 × 职业加成 |

**两套 XP 完全独立**：
- 互不影响，不存在转换关系
- 冒险 XP 的职业加成不影响现有 XP 的累积
- 猫咪进化仍由现有 XP 驱动（`total_minutes` 阈值）
- 冒险等级仅由冒险 XP 驱动

> **为什么不合并？** 现有 XP 系统已稳定运行，成就和进化逻辑依赖它。冒险 XP 需要职业加成乘数，而现有 XP 是纯累加。合并会引入不必要的耦合和回归风险。
