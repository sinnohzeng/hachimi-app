...
> - 2026-03-15 — 新增 SQLite 事务原子性保证（D40）；新增 §1.5 Guest 用户 DnD 功能行为及 uid 迁移规格
> - 2026-03-15 — crash 恢复策略改为原子事务（D40+D53）；stardustEarned 上限修正为 30（D30）；Guest migrateUid UNIQUE 冲突处理；新增 §8 Phase 1 前置改造 ActionType.tryFromValue（D38）
...
| 骰子检定中 App crash | SQLite 事务原子回滚，下次重新检定 | 无中间态 |

> **crash 恢复策略（D40 + D53）**：骰子检定的三步操作（删除 PendingDice + 写入 DiceResult + 更新星尘）在同一 SQLite 事务中原子执行。事务要么全成功要么全回滚，不存在"有结果无星尘"的中间态。无需额外的补发逻辑。
| 冒险中 App crash | AdventureProgress 状态保持 active | 用户继续未完成的冒险 |
| 数据损坏 | SQLite integrity_check → 自动重建 | "数据正在恢复..."（已有机制） |

> **SQLite 事务原子性保证（D40）**：骰子检定结算的三步操作（删除 PendingDice + 写入 DiceResult + 更新星尘）在同一 SQLite 事务中执行。事务要么全成功要么全回滚，不存在"有结果无星尘"或"有星尘无结果"的中间态。因此不需要额外的 `rewarded` 字段或 Ledger 级补发逻辑。Ledger 推送到远端的失败由现有 SyncEngine 指数退避重试机制覆盖。
...
### 1.5 Guest 用户 DnD 功能行为

> **核心原则**：Guest 用户享有与正式账号完全一致的 DnD 功能体验。

| 功能 | Guest 行为 | 说明 |
|------|-----------|------|
| 创建主哈基米 | ✅ 正常创建 | uid 使用 `guest_*` 前缀 |
| 获得/投掷骰子 | ✅ 正常获得和投掷 | 数据存于 SQLite |
| 冒险进度 | ✅ 正常推进 | 离线优先，与正式账号一致 |
| 职业选择 | ✅ 正常选择 | — |
| 金币/星尘 | ✅ 正常获取和消费 | — |
| 远端同步 | ❌ 不同步 | Guest 用户数据仅存本地，升级后通过 uid 迁移同步 |

#### Guest 升级 uid 迁移

Guest 升级为正式账号时，现有 `migrateUid` 机制需覆盖以下 6 张 DnD 新增表：

| 表名 | 迁移方式 |
|------|---------|
| `local_primary_cat` | `UPDATE local_primary_cat SET uid = ? WHERE uid = ?` |
| `local_pending_dice` | `UPDATE local_pending_dice SET uid = ? WHERE uid = ?` |
| `local_dice_results` | `UPDATE local_dice_results SET uid = ? WHERE uid = ?` |
| `local_adventure_progress` | `UPDATE local_adventure_progress SET uid = ? WHERE uid = ?` |
| `local_party` | `UPDATE local_party SET uid = ? WHERE uid = ?` |
| `local_scene_difficulty_unlock` | `UPDATE local_scene_difficulty_unlock SET uid = ? WHERE uid = ?` |

迁移在同一 SQLite 事务中执行，与现有 `local_habits`、`local_cats`、`local_sessions` 等表的迁移保持一致。

> **注意**：`local_primary_cat` 有 `UNIQUE(uid)` 约束。如果正式账号已有 PrimaryCat（理论上不可能，因为创建 PrimaryCat 在 Onboarding 中发生），迁移前需检查并处理冲突（last-write-wins）。
>
> **UNIQUE 约束冲突处理**：`UPDATE SET uid = ? WHERE uid = ?` 执行前，先检查目标 uid 是否已存在于该表中。若存在冲突（例如用户先以 Guest 使用后注册，但注册账号已有独立数据），以注册账号的数据为准，Guest 数据标记为废弃但不删除（留审计痕迹）。
...
> **PityState 持久化行为（D31 修正）**：保底计数器持久化到 `AdventureProgress.pityConsecutiveFailures` 和 `pityConsecutiveNonCrits` 字段。
> App 重启后从冒险进度恢复，不归零。冒险完成或放弃时重置为 0。
...
| `diceResults` | `stardustEarned` | `∈ [0, 30]` | 单次最大值含场景奖励和加成（D30） |
...
- [ ] **Crash 恢复一致性**：App crash 后重启，SQLite 事务已回滚，无中间态数据残留（D40+D53）
...
### Phase 1 前置改造：ActionType.tryFromValue()

**优先级**：Phase 1 首个 PR（D38）。

**改造范围**：
- `ActionType.fromValue()` 改为 `tryFromValue()` — 遇到未知值返回 `null` 而非抛出 `ArgumentError`
- 检查所有 `fromValue` 的调用点（位于 `LedgerAction.fromSqlite()` 中，被 sync_engine、ledger_service 等 10+ 个文件间接使用）
- 消费端需要处理 `null` 返回值：未知 ActionType 的 LedgerAction 应被静默跳过（log warning），不阻塞其他 action 的处理

**影响评估**：
- `LedgerAction.fromSqlite()` → 需要将 `ActionType` 字段改为 `ActionType?`
- `LedgerService.replay()` → 需要跳过 `actionType == null` 的记录
- `SyncEngine` → 同步推送时保留原始值，不丢弃未知类型
