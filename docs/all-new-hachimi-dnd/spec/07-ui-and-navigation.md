...
> **计数定义**：从 App 冷启动（Splash 完毕进入 Home）开始计数。每次 Tab 切换、页面跳转、按钮点击各计 1 次。目标路径：Home → Tab 1 → 选择习惯 → 开始专注 = 3 次。冒险流程（选场景→组队→投骰）是独立的可选路径，不受 3 次点击约束。
...
> **雷达图数据映射**：展示主哈基米的**原始属性值**（10-20）。六轴标签附带修正值（如 "STR 16 (+3)"）。图形中心 = 属性值 10（修正值 +0），最外圈 = 20（+5）。
...
> **catId 可选参数行为**：不传 `catId` 时，直接展示场景选择列表。传入 `catId` 时，自动跳到队伍编成并预选该猫为伙伴。
...
- 完成 Onboarding 的判断标准：`primaryCatProvider.valueOrNull != null`
- **不使用 SharedPreferences flag**：Onboarding 完成状态通过 `primaryCat != null` 推断（与 AppRouter redirect 逻辑一致），避免 SharedPreferences 与 Provider 状态不同步的风险
- App 启动时检查 `primaryCatProvider`，若为 null → 跳转至 `/starter-selection`
...
- [ ] Onboarding 完成后 `primaryCatProvider.valueOrNull != null`，重启不再触发
- [ ] Tab 1 专注完成后显示"+1 🎲"提示，不阻断用户操作
- [ ] 打开 App → 开始专注 ≤ 3 次点击（回归验证，不可破坏）
- [ ] 被动感知数值在 Tab 3 冒险者档案中显示
- [ ] Tab 2 发现事件气泡正确出现和消失
- [ ] Onboarding 展示 3 段进化（无 Senior）
- [ ] **Tab 3 骨架屏**：数据加载中显示 Shimmer 骨架屏（复用现有 SkeletonLoader），不显示空白页
- [ ] **路由参数安全**：所有路由参数使用 GoRouter 的 queryParams，特殊字符（中文、空格）正确 URL 编码

---

## 附录 A：SRD 深度借鉴扩展 — UI 新增项

### A.1 新增路由

| 路由 | 用途 | Phase |
|------|------|-------|
| `/inventory` | 物品库存管理（装备、药水、Trinkets） | 3 |

### A.2 Tab 2（酒馆）扩展

- **NPC 面板**（Phase 3）：酒馆底部新增 NPC 互动区域（6 位 NPC，详见 spec/11 §9）
- **制作台入口**（Phase 3+）：酒馆中新增「制作台」按钮（详见 spec/12 §6）

### A.3 Tab 3（冒险者日志）扩展

- **Trinket 图鉴**（Phase 2）：底部折叠区域，网格展示已收集/未发现（详见 spec/09 §6.5）
- **休息状态指示器**（Phase 2）：冒险者档案旁显示"长休生效"/"短休生效"徽章（详见 spec/10 §3.4）
- **状态效果栏**（Phase 2）：冒险进行界面顶部水平图标条（详见 spec/08 §6.1）
