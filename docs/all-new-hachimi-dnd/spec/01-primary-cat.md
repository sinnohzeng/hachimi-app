...
> - 2026-03-15 — 编码前全面修复：extension 改为静态方法（D52）、Service 定义归属 spec/02（D56）、新增 background bonus 层
...
"优势属性"加成（+2）和"背景加成"（+1）均在 `PrimaryCatAbilities` 静态方法中以常量映射表实现，不用 if/else 分支。背景加成定义见 [spec/11-social-and-origins.md](11-social-and-origins.md)。
...
**属性字段不存储** — 全部由 `PrimaryCatAbilities` 静态方法在运行时从 `primaryCatAbilitiesProvider` 数据聚合计算（D52）。
...
文件路径：`lib/models/primary_cat_abilities.dart`

> **设计决策（D52）**：计算编排在 `primaryCatAbilitiesProvider` 中完成，Provider 负责收集各项输入数据并调用下方静态方法。`PrimaryCatAbilities` 仅保留纯公式逻辑，各方法显式接收所有必要参数，不依赖隐式注入。

```dart
/// 主哈基米属性计算 — 纯静态方法，无副作用
class PrimaryCatAbilities {
  PrimaryCatAbilities._(); // 禁止实例化

  /// STR：全部伙伴猫 totalMinutes 之和，每 120 分钟 +1，基础 10
  static int calcStrength(int allCatsTotalMinutes, String archetype, String background) =>
      _applyBackgroundBonus('str', background,
        _applyBonus('str', archetype, _clamp(_minutesToScore(allCatsTotalMinutes))));

  /// DEX：近 30 次专注的 completionRatio 均值驱动
  static int calcDexterity(double overallCompletionRate, String archetype, String background) =>
      _applyBackgroundBonus('dex', background,
        _applyBonus('dex', archetype, _clamp(_efficiencyToScore(overallCompletionRate))));

  /// CON：所有习惯合并后的历史最长连续打卡天数
  static int calcConstitution(int longestEverStreak, String archetype, String background) =>
      _applyBackgroundBonus('con', background,
        _applyBonus('con', archetype, _clamp(_streakToScore(longestEverStreak))));

  /// INT：活跃习惯数量 + 平均目标时长的复合分
  static int calcIntelligence(int activeHabitCount, double avgGoalMinutes, String archetype, String background) =>
      _applyBackgroundBonus('int', background,
        _applyBonus('int', archetype, _clamp(_habitDiversityToScore(activeHabitCount, avgGoalMinutes))));

  /// WIS：最近 30 天内有专注记录的天数占比
  static int calcWisdom(double thirtyDayCoverage, String archetype, String background) =>
      _applyBackgroundBonus('wis', background,
        _applyBonus('wis', archetype, _clamp(_consistencyToScore(thirtyDayCoverage))));

  /// CHA：所有活跃伙伴猫的平均心情分
  static int calcCharisma(double averageCatMood, String archetype, String background) =>
      _applyBackgroundBonus('cha', background,
        _applyBonus('cha', archetype, _clamp(_happinessToScore(averageCatMood))));
  ...
  static int _applyBonus(String attr, String archetype, int base) {
    final bonusAttrs = _archetypeBonusMap[archetype] ?? {};
    return bonusAttrs.contains(attr) ? (base + 2).clamp(10, 20) : base;
  }

  /// 背景加成 +1，通过 Map 查表（定义见 spec/11-social-and-origins.md）
  static const _backgroundBonusMap = {
    'scholar':   'int',
    'athlete':   'str',
    'healer':    'wis',
    'performer': 'cha',
  ...
  static int _applyBackgroundBonus(String attr, String background, int base) {
    final bonusAttr = _backgroundBonusMap[background];
    if (bonusAttr == null || bonusAttr != attr) return base;
    return (base + 1).clamp(10, 20);
  }

  static int _clamp(int raw) => raw.clamp(10, 20);
}
```
...
**属性计算三层叠加顺序**：`基础公式分` → `_applyBonus`（原型加成 +2）→ `_applyBackgroundBonus`（背景加成 +1）→ 最终 `clamp(10, 20)`。

### 各公式输入的数据来源（关键）

以下输入值由 `primaryCatAbilitiesProvider` 在调用静态方法前收集，不由计算方法自行查数据库。`archetype` 和 `background` 从 `PrimaryCat` 模型直接读取，作为每个 `calc*` 方法的必需参数。
...
| 文件路径 | 职责 |
|---------|------|
| `lib/models/primary_cat.dart` | PrimaryCat 数据模型 + PrimaryCatMood extension |
| `lib/models/primary_cat_abilities.dart` | PrimaryCatAbilities 纯静态属性计算类（D52） |
...
**属性计算依赖的 3 个 Service**（CompletionRateService、StreakService、CoverageService）的 SSOT 定义见 [spec/02-dnd-attributes.md](02-dnd-attributes.md)（D56）。
