...
> - 2026-03-15 — exhausted 替换为 well_rested（零惩罚原则 D21）：正面 6 种、负面 2 种；更新交互矩阵和 conditionModifier 示例
...
- **正面为主**：8 种状态中正面 **6** 种、负面 **2** 种，正面状态触发频率远高于负面
...
| 精力充沛 | `well_rested` | Exhaustion（反转） | 所有检定 **+1** | 连续 ≥ 3 天每天有完成至少 1 个习惯 | 整次冒险 | — |

> **零惩罚原则（D21）**：原 `exhausted`（不活跃惩罚）改为 `well_rested`（活跃奖励），
> 消除"不打开 App 会受惩罚"的违规。不活跃用户回到基线，活跃用户获得 buff。
...
class ActiveCondition {
  ...
  final String conditionType;     // 'inspired' | 'focused' | 'energized' | 'lucky' | 'blessed' | 'well_rested' | 'frightened' | 'poisoned'
...
例：energized(+1) + focused(+2) + well_rested(+1) + poisoned(-1) = conditionModifier = +3
```
...
| 状态效果 | 药水 | 治愈药水清除 poisoned；专注药水附加 focused |
| 状态效果 | 陷阱 | 陷阱失败附加 poisoned / frightened |
