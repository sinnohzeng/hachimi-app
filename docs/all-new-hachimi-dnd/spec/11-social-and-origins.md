...
> - 2026-03-15 — 新增 Phase 1 adventurer 默认值说明（D51）；NPC 互动 CHA 检定公式引用（spec/03 §6.5）
...
> **Phase 1 默认值（D51）**：Phase 1 中 `PrimaryCat.background` 默认为 `'adventurer'`，这是非用户可选的占位值，**不提供任何属性加成**。Phase 3 启用社交系统后，在 Onboarding 中新增背景选择步骤，用户从上述 4 种背景中选择替换 `'adventurer'`。
...
> **NPC 互动检定**：NPC 互动中的 CHA 检定使用与冒险场景相同的完整检定公式（spec/03 §6.5），包含伙伴猫的属性匹配加成和性格优势判定。
