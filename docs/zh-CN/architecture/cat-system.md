...
| `growthScore` | 0.20 | 成长激励 | stageProgress>=0.85: 1.0, >=0.70: 0.7, 其他: stageProgress*0.5 |
| `todayScore` | 0.05 | 今日完成 | 未做: 1.0, 已做: 0.0 |

**行为推演：**
- 刚完成专注 → recency（1.0 × 0.45）主导 → 显示刚陪伴的猫
- 一周没开 app → mood（1.0 × 0.30）主导 → 显示最想念用户的猫
- 全 adult → recency + mood 仍有效 → 不退化为盲选
- 单猫 → 短路返回 `cats.first`
...
Hachimi 日记赋予每只猫每天撰写日记的能力，日记内容基于用户的习惯完成情况、猫咪的性格和心情。AI 对已认证用户始终开启——无需手动启用。
...
用户可以与猫咪进行文字对话。猫咪根据其性格特征进行角色扮演式回复。AI 对已认证用户始终开启——无需手动启用。当 AI 不可用时，聊天页面显示 `_AiOfflineBanner`。

### 每日消息限额

每只猫每天最多 **5 条消息**（通过 RemoteConfig 键 `chat_daily_limit` 可配置，默认值：5）。采用纵深防御：服务层在达到限额时拒绝消息，UI 通过 `getRemainingMessages()` 禁用发送按钮。
...
一个轻量聊天页面，位于 **设置 → AI → 「测试模型」**，用户可快速验证云端 AI 连接是否正常工作，无需导航至特定猫咪页面。

### 用途

用户需要一种零摩擦的方式确认云端 AI 连接正常。测试聊天提供了这个功能。
...
| 服务层 | ChatService（含历史记录） | 通过 AiProvider 接口调用 AiService |
