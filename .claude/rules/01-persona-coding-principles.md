...
- **只在系统边界验证**（用户输入、Firebase API、外部服务响应），信任内部函数契约
- **禁止防御性冗余**：不在内部函数间添加"以防万一"的检查（如多余的 null check、类型断言、范围校验）
- **删除不可能场景的处理**：不为不可能发生的错误场景编写处理代码
...
9. **不越界提方案**：严格只解决用户明确提出的需求，禁止主动添加兜底方案、降级逻辑、预防性代码或任何"顺便做"的优化。如果认为某个额外方案有价值，必须先提出讨论，等用户确认后再纳入实现
